(function() {
  ...
  if (form) {
    ...
    var contactPresets = {
      'dag-sport': {
        ...
        scenario: 'task',
        message: 'Хочу разобрать интеграцию существующего сайта с 1С. Сейчас есть сайт или магазин и учёт в 1С. Нужно определить состав данных, направления обмена и подходящий способ интеграции.'
      }
    };
  ...
  if (workRecommend) {
    ...
    var workRecommendMap = {
      ...
      'one-c': {
        copy: '1С-связка: Dag Sport. Существующий сайт на 1С-Битрикс связан с 1С УТ; моя часть — интеграция и AI-консультант.'
      }
    };
  ...
  document.querySelectorAll('[data-before-after]').forEach(function(block) {
    ...
    function updateBeforeAfter() {
      ...
      var valueText = 'Исходная система: ' + before + '%, моя часть: ' + after + '%';

      if (before === 0) valueText = 'Полностью моя часть: интеграция с 1С УТ и AI-консультант';
      if (before === 50) valueText = 'Исходная система и моя часть поровну';
      if (before === 100) valueText = 'Полностью исходная система: существующий сайт на 1С-Битрикс';
  ...
  if (projectSheet) {
    ...
    var projectDetails = {
      ...
      monitor: {
        kicker: 'Telegram · мониторинг',
        title: 'MyServerAgentAI',
        lead: 'Telegram-ассистент следит за моими сайтами и сервером: проверяет доступность, ресурсы и SSL, присылает сводки и сообщает о сбоях.',
        task: 'Получать понятный статус инфраструктуры и предупреждение о проблеме до обращения клиента.',
        route: 'Проверка → сводка в Telegram → уточнение в чате → перезапуск сервиса при необходимости.',
        repeat: 'Подходит для внутренних инструментов, где важно быстро понять, что работает, что требует внимания и какое действие выполнить.',
        link: '',
        scenario: 'ai'
      },
      dag: {
        ...
        title: 'Dag Sport',
        lead: 'Сайт на 1С-Битрикс уже существовал; моя часть — интеграция с 1С УТ и AI-консультант.',
        task: 'Связать существующий сайт на 1С-Битрикс с 1С УТ и добавить AI-консультанта по каталогу.',
        route: 'Факт интеграции с 1С УТ подтверждён. По словам владельца, заказы не теряются; точный механизм и направления обмена без проверки не заявляю.',
        repeat: 'Подходит, когда сайт уже есть и нужно отдельно оценить интеграцию с 1С и AI-сценарий.',
